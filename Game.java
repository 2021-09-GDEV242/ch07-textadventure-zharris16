...
public class Game 
{
    ...
    private void createRooms()
    {
        ...
        complab.setExit("west", complabAdmin);
        ...
        tennisCourt.setExit("west", courtyard);
        tennisCourt.setExit("north", badmittenCourt);

        badmittenCourt.setExit("south", tennisCourt);
        badmittenCourt.setExit("west", frontGate);

        weightRoom.setExit("east", courtyard);
        weightRoom.setExit("north", coachingOffice);

        coachingOffice.setExit("south", weightRoom);
        coachingOffice.setExit("east", frontGate);
    ...
    private void goRoom(Command command) 
    {
        ...
        } else if(nextRoom.getShortDescription().equals("at the front gate/exit")){
            if(player.getInventoryItem("Keycard") != null){
                System.out.println("You have reached the frontgate with the Keycard!");
                System.out.println("Thank you for playing.  Good bye.");
                System.exit(0);
            } else{
                System.out.println("Front gate is locked, find the keycard");
            }
        } else {
     ...
     * @param Command command: input command.
     */
    private void take(Command command) 
    {
        ...
        // Try to leave current room.
        Item takenItem = currentRoom.takeRoomItem(item);
